...
def registerUser(request):
    if request.user.is_authenticated:
        check_role_vendor(request.user)
        messages.warning(request,"You are already logged in as a customer! as a customer")
...
def registerVendor(request):
    if request.user.is_authenticated:
        check_role_vendor(request.user)
        messages.warning(request,"You are already logged in! as a vendor")
        return redirect('myAccount')
    elif request.method=='POST':
